...
class ReplaceTextModuleTest : StringSpec({
    ...
    "should return OperationNotNeededModuleResponse when the title of the link is not a ticket ID" {
        val request = ReplaceTextModule.Request(
            42L,
            listOf(
                ReplaceTextModule.Comment(100L, "[A similar issue with this was fixed previously|https://bugs.mojang.com/browse/MC-4]") { Unit.right() }
            )
        )
    ...
    "should return OperationNotNeededModuleResponse when the title of the link is not the same ticket as specified in the /projects link" {
        val request = ReplaceTextModule.Request(
            42L,
            listOf(
                ReplaceTextModule.Comment(100L, "[MC-5|https://bugs.mojang.com/projects/MC/issues/MC-4]") { Unit.right() }
            )
        )
    ...
    "should replace /projects links with query" {
        ...
        val request = ReplaceTextModule.Request(
            42L,
            listOf(
                ReplaceTextModule.Comment(100L, "Duplicates https://bugs.mojang.com/projects/MC/issues/MC-4?jql=votes%3E0") {
                    replacedCommentBody = it
                    Unit.right()
                }
            )
