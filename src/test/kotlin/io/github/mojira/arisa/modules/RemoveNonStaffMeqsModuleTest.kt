...
class RemoveNonStaffMeqsModuleTest : StringSpec({
    ...
    "should return OperationNotNeededModuleResponse when there is no comments" {
        val module = RemoveNonStaffMeqsModule({ _, _ -> Unit.right() }, { false.right() }, { Unit.right() })
    ...
    "should return OperationNotNeededModuleResponse for a staff restricted comment" {
        val module = RemoveNonStaffMeqsModule({ _, _ -> Unit.right() }, { true.right() }, { Unit.right() })
    ...
    "should return OperationNotNeededModuleResponse when checking visibility fails" {
        val module = RemoveNonStaffMeqsModule({ _, _ -> Unit.right() }, { RuntimeException().left() }, { Unit.right() })
        val comment = mockComment("MEQS_WAI I like QC.")
        val request = RemoveNonStaffMeqsModuleRequest(listOf(comment))
    ...
    "should return FailedModuleResponse when updating body fails" {
        val module = RemoveNonStaffMeqsModule({ _, _ -> RuntimeException().left() }, { false.right() }, { Unit.right() })
        val comment = mockComment("MEQS_WAI I like QC.")
        val request = RemoveNonStaffMeqsModuleRequest(listOf(comment))
    ...
    "should return FailedModuleResponse with all exceptions when updating body fails" {
        val module = RemoveNonStaffMeqsModule({ _, _ -> RuntimeException().left() }, { false.right() }, { Unit.right() })
        val comment = mockComment("MEQS_WAI I like QC.")
    ...
    "should return FailedModuleResponse when updating visibility fails" {
        val module = RemoveNonStaffMeqsModule({ _, _ -> Unit.right() }, { false.right() }, { RuntimeException().left() })
        val comment = mockComment("MEQS_WAI I like QC.")
        val request = RemoveNonStaffMeqsModuleRequest(listOf(comment))
    ...
    "should return FailedModuleResponse with all exceptions when updating visibility fails" {
        val module = RemoveNonStaffMeqsModule({ _, _ -> Unit.right() }, { false.right() }, { RuntimeException().left() })
        val comment = mockComment("MEQS_WAI I like QC.")
        val request = RemoveNonStaffMeqsModuleRequest(listOf(comment, comment))
    ...
    "should only remove MEQS of the comment" {
        val module = RemoveNonStaffMeqsModule({ _, body -> body.shouldBe("_WAI I like QC.").right() }, { false.right() }, { Unit.right() })
        val comment = mockComment("MEQS_WAI I like QC.")
        val request = RemoveNonStaffMeqsModuleRequest(listOf(comment))
