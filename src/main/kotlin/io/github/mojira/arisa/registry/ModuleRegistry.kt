...
abstract class ModuleRegistry(private val config: Config) {
    data class Entry(
        val name: String,
    ...
    protected abstract val getJql: (Instant) -> String
    ...
    fun getFullJql(lastRun: Instant, failedTickets: Collection<String>): String {
        ...
        val failedTicketsJql = if (failedTickets.isEmpty()) "key in (${ failedTickets.joinToString() })" else ""
        val registryJql = "(${ getJqlWithDebug(lastRun) })"

        return "project IN (${ projects.joinToString() } AND ($failedTicketsJql OR $registryJql) ORDER BY updated ASC"
    }
